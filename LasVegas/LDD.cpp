...
vector<vector<int>> LDD(Graph &g, int d)
{
    ...
    {
        if (g.adjacencyList[v].size() >= 1 || g_rev.adjacencyList[v].size() >= 1)
        {
            s = v;
            foundGoodS = true;
            break;
                                   ...
                                   int numSettled, vector<vector<int>> &farthestDistancesSeen, double constant, vector<int> &dist, int d)
{
    if (pq.empty())
    {
        ...
        int i_big = farthestDistanceSeen + (int)ceil(constant);
